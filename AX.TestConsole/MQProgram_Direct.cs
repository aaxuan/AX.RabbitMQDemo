...
{
    //direct 模式 基于交换机加上路由Key实现分发给不同的频道

    internal class MQProgram_Direct
    {
        public void Run()
        {
        ...
        public void Produce()
        {
            ...
            string exchangeName = $"{GetType().Name}_exchange_direct_生产者";
            ...
            try
            {
                ...
                //绑定关系 加上路由key
                channel.QueueBind($"{GetType().Name}_生产者_1", exchangeName, "user");
                channel.QueueBind($"{GetType().Name}_生产者_2", exchangeName, "bill");
                channel.QueueBind($"{GetType().Name}_生产者_3", exchangeName, "message");

                var count = new System.Random().Next(1, 50);
                Console.WriteLine(count);
                for (int i = 0; i < count; i++)
                {
                    channel.BasicPublish(exchangeName, "bill", body: Encoding.UTF8.GetBytes($"【{DateTime.Now.ToString("HH:mm:ss.fffffff")}】 【{i}】 hello world"));
                }
            }
            catch (Exception ex)
            {
